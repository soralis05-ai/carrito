...
class Category(db.Model):
    ...
    slug: db.Column = db.Column(db.String(100), unique=True, nullable=False, index=True)
    description: db.Column = db.Column(db.Text, nullable=True)
    is_active: db.Column = db.Column(db.Boolean, default=True)
    created_at: db.Column = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
...
class Product(db.Model):
    ...
    id: db.Column = db.Column(db.Integer, primary_key=True)
    name: db.Column = db.Column(db.String(200), nullable=False, index=True)
    slug: db.Column = db.Column(db.String(200), unique=True, nullable=False, index=True)
    description: db.Column = db.Column(db.Text, nullable=True)
    price: db.Column = db.Column(db.Numeric(10, 2), nullable=False)
    stock: db.Column = db.Column(db.Integer, default=0)
    sku: db.Column = db.Column(db.String(50), unique=True, nullable=True, index=True)

    # Imágenes (almacena JSON con lista de nombres de archivo)
    image_main: db.Column = db.Column(db.String(255), nullable=True)  # Imagen principal
    images: db.Column = db.Column(db.JSON, nullable=True)  # Lista de imágenes adicionales

    # Costos de producción (almacena JSON con estructura de costos)
    costos: db.Column = db.Column(db.JSON, nullable=True)  # Costos de amigurumis

    # Categoría
    category_id: db.Column = db.Column(db.Integer, db.ForeignKey('categories.id'), nullable=True, index=True)

    # Estado
    is_active: db.Column = db.Column(db.Boolean, default=True)
    is_featured: db.Column = db.Column(db.Boolean, default=False)
    created_at: db.Column = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
